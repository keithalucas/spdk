...
snapshot_checksum(void)
{
	...
	/* Make a blob's snapshot without the option to add new xattrs after its creation */
	ut_spdk_blob_opts_init(&blob_opts);
	spdk_bs_create_snapshot_ext(bs, blobid, &blob_opts, blob_op_with_id_complete, NULL);
	poll_threads();
	CU_ASSERT(g_bserrno == 0);
	snapid1 = g_blobid;
	...
	/* Try to compute and store checksum of a snapshot without the option for xattr add only */
	spdk_bs_snapshot_checksum(bs, blob_ch, snapid1, xattr_name, blob_op_complete, NULL);
	poll_threads();
	CU_ASSERT(g_bserrno == -EPERM);

	ut_blob_close_and_delete(bs, snap1);

	/* Make a blob's snapshot with the option to add new xattrs after its creation */
	ut_spdk_blob_opts_init(&blob_opts);
	blob_opts.enable_add_xattrs = true;
	spdk_bs_create_snapshot_ext(bs, blobid, &blob_opts, blob_op_with_id_complete, NULL);
	poll_threads();
	CU_ASSERT(g_bserrno == 0);
	snapid2 = g_blobid;
	...
	CU_ASSERT(g_bserrno == 0);
	snap2 = g_blob;
	CU_ASSERT(snap2->md_ro == true);

	/* Compute and store checksum of a snapshot */
	spdk_bs_snapshot_checksum(bs, blob_ch, snapid2, xattr_name, blob_op_complete, NULL);
	poll_threads();
	CU_ASSERT(g_bserrno == -0);

	/* Get checksum */
	rc = spdk_blob_get_xattr_value(snap2, xattr_name, &value, &value_len);
	CU_ASSERT(rc == 0);
