...
int g_lvolerrno;
int g_lvserrno;
int g_cluster_size = SPDK_BS_PAGE_SIZE;
int g_num_clusters = 20;
...
static void
vbdev_quiesce_complete(void *cb_arg, struct spdk_lvol *lvol, int lvolerrno)
{
	g_lvolerrno = lvolerrno;
}
...
ut_lvol_resize(void)
{
	...
	g_lvolerrno = -1;
	vbdev_lvol_resize(lvol, g_num_clusters, vbdev_lvol_resize_complete, NULL);
	CU_ASSERT(g_lvolerrno == 0);
	CU_ASSERT(lvol->bdev->blockcnt == g_num_clusters * g_cluster_size / lvol->bdev->blocklen);

	/* Resize with NULL lvol */
	vbdev_lvol_resize(NULL, g_num_clusters, vbdev_lvol_resize_complete, NULL);
	CU_ASSERT(g_lvolerrno != 0);
...
ut_lvol_quiesce(void)
{
	...
	lvol = g_lvol;
	io_ch = _vbdev_lvol_get_io_channel(lvol);
	lvol_ch = (struct spdk_lvol_channel *)spdk_io_channel_get_ctx(io_ch);

	/* Allocate io and set lvol as context */
	g_io = calloc(1, sizeof(struct spdk_bdev_io) + vbdev_lvs_get_ctx_size());
