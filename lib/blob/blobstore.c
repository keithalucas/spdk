...
bs_set_parent_snapshot_open_cpl(void *cb_arg, struct spdk_blob *snapshot, int bserrno)
{
	...
	struct spdk_blob *blob = ctx->blob;
	struct spdk_bs_dev *back_bs_dev;
	struct blob_parent_id *parent_id;
	...
	parent_id = calloc(1, sizeof(*parent_id));
	if (!parent_id) {
		ctx->bserrno = -ENOMEM;
		bs_set_parent_close_blob(ctx, ctx->bserrno);
		return;
	}
	parent_id->u.snapshot.id = snapshot->id;

	blob_set_back_bs_dev(blob, back_bs_dev, bs_set_parent_refs, parent_id, bs_set_parent_unfrozen, ctx);
}
