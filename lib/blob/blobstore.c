...
blob_alloc(struct spdk_blob_store *bs, spdk_blob_id id)
{
	...
	blob->u.shallow_copy.cluster_copy_index = 0;
...
uint64_t
spdk_blob_get_shallow_copy_cluster_index(struct spdk_blob *blob)
{
	assert(blob != NULL);

	if (blob->u.shallow_copy.num_clusters_to_copy > 0) {
		return blob->u.shallow_copy.cluster_copy_index;
	} else {
		return UINT64_MAX;
	}
}
...
bs_shallow_copy_bdev_write_cpl(struct spdk_io_channel *channel, void *cb_arg, int bserrno)
{
	...
	if (bserrno != 0) {
		...
		_blob->u.shallow_copy.cluster_copy_index = 0;
...
bs_shallow_copy_cluster_find_next(void *cb_arg, int bserrno)
{
	...
	if (ctx->cluster < _blob->active.num_clusters) {
		_blob->u.shallow_copy.cluster_copy_index = ctx->cluster;
